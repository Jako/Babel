...
:fontawesome-solid-square-plus: icon, remove all links to the source resource
with a click on the :fontawesome-solid-link-slash: icon or delete all linked
resource of the source resource with a click on the
:fontawesome-solid-trash-can: icon. For each translatable context a grid column
is available with two icons. The following icons are available depending on the
link state:
